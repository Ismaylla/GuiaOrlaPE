...
interface HeaderProps {
  ...
  isEmpreendedor?: boolean; // Nova prop
}
...
export const HeaderListagem = ({ 
  ...
  const filterBtnColor = isEmpreendedor ? 'bg-[#0A4F6E]' : 'bg-[#FF7620]'; 
  const profileIcon = isEmpreendedor ? '/icons/PerfilAzul.svg' : '/icons/Perfil.svg';

  return (
    <header className={`${bgColor} w-full fixed top-0 z-50 transition-all duration-500 shadow-md flex flex-col justify-center ${scrolled ? "h-auto py-2 md:h-16" : "h-24"}`}>
      <div className="w-full flex items-center justify-between px-4 md:px-10 gap-2">
        ...
        <div className={`items-center gap-4 transition-all duration-500 shrink-0 hidden md:flex ${scrolled ? "opacity-100 ml-4" : "opacity-0 w-0 overflow-hidden"}`}>
          ...
          <Image src={profileIcon} alt="Perfil" width={32} height={32} className="shrink-0 cursor-pointer" />
        </div>
      </div>
      <div className={`md:hidden flex items-center gap-2 px-4 py-2 transition-all duration-500 ${scrolled ? "opacity-100 h-auto" : "opacity-0 h-0 overflow-hidden"}`}>
        ...
        <button onClick={() => setIsFilterOpen(true)} className={`${filterBtnColor} text-white px-2 py-1.5 rounded-lg text-[9px] font-bold whitespace-nowrap shadow-sm`}>⚙️ FILTROS</button>
        ...
        <Image src={profileIcon} alt="Perfil" width={28} height={28} className="shrink-0" />
      </div>
    </header>
  );
};