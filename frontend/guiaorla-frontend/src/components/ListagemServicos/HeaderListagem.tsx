...
//   forceBlue?: boolean; 
//   showFilter?: boolean; // NOVA PROP: Para controlar se o botão de filtro aparece
// }
...
//   forceBlue = false,
//   showFilter = true // Valor padrão é true para não quebrar a home
// }: HeaderProps) => {
...
//             {/* CONDICIONAL: Só mostra o filtro se showFilter for true */}
//             {showFilter && (
//               <button 
    ...
    <header className={`${bgColor} w-full fixed top-0 z-50 shadow-md h-16 flex items-center px-4 transition-colors duration-300`}>
      <div className="max-w-7xl w-full mx-auto flex items-center gap-3">
        ...
        <nav 
          ref={navRef} 
          onMouseDown={handleMouseDown(navRef)}
          className="flex-1 flex items-center overflow-x-auto scrollbar-hide flex-nowrap cursor-grab gap-3 ml-2 touch-pan-x"
        >
          {["Barracas e Ambulantes", "Passeios e Lazer", "Bares e Restaurantes", "Artesanato Local", "Comércio e Serviços"].map((cat) => (
            <button 
              key={cat} 
