...
export const PerfilHeader = ({ podeEditar = false }: PerfilHeaderProps) => {
  return (
    <div className="bg-white shadow-sm rounded-b-2xl overflow-hidden border-x border-b border-gray-200">
      ...
      <div className="relative h-[180px] md:h-[300px] w-full bg-gradient-to-r from-gray-200 to-gray-300">
        ...
        {/* Renderização Condicional da Câmera */}
        {podeEditar && (
          <button className="absolute bottom-4 right-4 bg-white/80 backdrop-blur-md p-2 rounded-lg flex items-center gap-2 text-sm font-semibold text-[#0A4F6E] hover:bg-white transition-all shadow-sm">
            <Camera size={18} />
      ...
      <div className="max-w-[1050px] mx-auto px-4 pb-6">
        <div className="relative flex flex-col md:flex-row items-center md:items-end gap-5 -mt-10 md:-mt-14">
          ...
          {/* FOTO DE PERFIL */}
          <div className="relative h-28 w-28 md:h-36 md:w-36 rounded-full border-4 border-white bg-gray-100 shadow-md overflow-hidden shrink-0">
            <Image 
              ...
              className="object-cover"
            />
            {/* Opcional: Adicionar ícone de câmera aqui também se podeEditar for true */}
          </div>
