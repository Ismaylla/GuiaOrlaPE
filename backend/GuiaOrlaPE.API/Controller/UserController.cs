...
public class UserController : ControllerBase
{
    ...
    public async Task<IActionResult> CreateBusinessperson([FromBody] CreateBusinesspersonRequest request)
    {
        try
        {
            ...
            var response = new
            {
                ...
                user.Status,
                user.CreatedAt,
                Businesses = user.Businesses.Select(b => new { b.Id, b.Name, b.ServiceType, b.Address })
            };
    ...
    public async Task<IActionResult> Login([FromBody] LoginRequest request)
    {
        try
        {
            ...
            var usuarioValido = users.FirstOrDefault(u => u.Email.ToLower() == request.Email.ToLower());

            if (usuarioValido == null)
                return BadRequest(new { Message = "E-mail não encontrado no banco de dados." });

            var senhaValida = BCrypt.Net.BCrypt.Verify(request.Password, usuarioValido.PasswordHash);
            if (!senhaValida)
