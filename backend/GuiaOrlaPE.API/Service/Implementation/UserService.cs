...
//         var user = await _repository.GetByIdAsync(userId) ?? throw new Exception("Usuário não encontrado.");

//         // 2. Verifica se o novo e-mail já está em uso por OUTRA pessoa
//         var existingUserWithEmail = await _repository.GetByEmailAsync(newEmail);
...
//         // 3. Atualiza o e-mail e salva pelo repositório
//         user.Email = newEmail;
//         await _repository.UpdateAsync(user);
//     }
...
public class UserService(IUserRepository repository, ITokenService tokenService, IEmailService emailService) : IUserService
{
    ...
    public async Task ChangePasswordAsync(Guid userId, string currentPassword, string newPassword)
    {
        ...
        // Verifica se a senha atual digitada bate com a que está no banco
        var passwordIsValid = BCrypt.Net.BCrypt.Verify(currentPassword, user.PasswordHash);
        if (!passwordIsValid)
        {
    ...
    // =========================================================================
    // EXCLUIR CONTA (HARD DELETE - APAGA DO BANCO DE VERDADE)
    // =========================================================================
